...
const AddProduct = () => {
  ...
  useEffect(() => {
    setLoader(true);
    fetch(`https://server-12-tau.vercel.app/users/${email}`)
      .then((res) => res.json())
      .then((data) => {
        return setAuthor(data), setLoader(false);
      });
  }, [email]);
  ...
  const onSubmit = (data) => {
    ...
    fetch(
      `https://api.imgbb.com/1/upload?key=8127da087d2a44fbea2792aba3cdee01`,
      {
        method: "POST",
        body: formData,
      }
      ...
      .then((imgData) => {
        if (imgData.success) {
          ...
          fetch(`https://server-12-tau.vercel.app/addProduct`, {
            method: "POST",
            headers: {
            ...
            .then((data) => {
              if (data.acknowledged) {
                ...
                setIsloader(false);
                toast.success(
                  "your product hasbeen process please waite for admin aproved"
                );
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="form-control">
            ...
            <select
              className={
                errors.category
                  ? "input-bordered input-error select select-bordered w-full"
          ...
          <div className="form-control mt-5">
            <label className="label">
              <span className="label-text inline-block mb-2 text-gray-700">
                Description
              </span>
            </label>
            <textarea
          ...
          <div className="form-control mt-6">
            <button
              type="submit"
              className={
                isloader ? "btn btn-neutral loading" : "btn btn-neutral"
              }
            >
