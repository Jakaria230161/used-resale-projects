...
const AllSeller = () => {
  const { data: sellers = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://server-12-tau.vercel.app/allUser/Seller`
      );
  ...
  const handleDeleteDoctor = (seller) => {
    fetch(`https://server-12-tau.vercel.app/sellers/${seller._id}`, {
      method: "DELETE",
      headers: {
  ...
  const handleMakeVerify = (id) => {
    fetch(`https://server-12-tau.vercel.app/users/seller/${id}`, {
      method: "PUT",
      headers: {
        authorization: `bearer ${localStorage.getItem("accessToken")}`,
      },
          ...
          <tbody>
            {sellers.map((seller, i) => (
              <tr key={seller._id}>
                ...
                <td>
                  <button
                    onClick={() => handleDeleteDoctor(seller)}
