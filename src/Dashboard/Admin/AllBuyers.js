...
const AllBuyers = () => {
  const { data: buyers = [], refetch } = useQuery({
    queryKey: ["users"],
    queryFn: async () => {
      const res = await fetch(`https://server-12-tau.vercel.app/allUser/Buyer`);
  ...
  const handleDeleteDoctor = (buyer) => {
    ...
    fetch(`https://server-12-tau.vercel.app/buyers/${buyer._id}`, {
      method: "DELETE",
      headers: {
          ...
          <tbody>
            {buyers.map((buyer, i) => (
              <tr key={buyer._id}>
                ...
                <td>
                  <button
                    onClick={() => handleDeleteDoctor(buyer)}
